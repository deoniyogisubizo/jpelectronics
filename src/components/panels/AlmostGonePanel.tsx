...
export default function AlmostGonePanel() {
  ...
  return (
    <section className="py-2 bg-beige border-l-4 border-black/20">
      <div className="container mx-auto">
          ...
          <Link href={`/product/${heroProduct._id}`}>
            <div className="bg-white/60 backdrop-blur-sm rounded-lg overflow-hidden shadow-sm border border-gray-100">
              <div className="relative aspect-[3/4] overflow-hidden bg-beige-solid">
                ...
                <span className="absolute top-2 right-2 bg-black text-white text-xs px-2 py-1 rounded-full font-semibold">{heroProduct.stockQuantity} left</span>
              </div>
              <div className="p-4">
                ...
                <div className="mb-3">
                  ...
                  {heroProduct.compareAtPrice && heroProduct.compareAtPrice > heroProduct.price && (
                    <span className="bg-gray-200 text-gray-500 text-sm px-2 py-0.5 rounded ml-2" style={{ fontFamily: 'var(--font-share-tech-mono)' }}>{formatPrice(heroProduct.compareAtPrice)}</span>
                  )}
                </div>
                <button className="w-full py-2 bg-black text-white rounded-lg font-medium hover:bg-black/80 transition-colors text-sm">Shop Now — Limited Stock!</button>
              </div>
            </div>
          </Link>
